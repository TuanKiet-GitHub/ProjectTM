...
namespace ThapMuoi.Services.Core
{
    public class MenuService : BaseService, IMenuService
    {
        ...
        public async Task<dynamic> AddAC(MenuList model)
        {
            ...
            var update = Builders<Menu>.Update.AddToSet(x => x.ListAction, actionName);
        ...
        public async Task<dynamic> DeleteAc(MenuListShort model)
        {
            try
            {
                if (model == default)
                ...
                {
                    entity.ListAction.Remove(model.ListAction);
